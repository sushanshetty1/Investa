...
const Navbar = () => {
  ...
  const handleAuthAction = async (action: string) => {    if (action === "login") {
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">        <div className="flex items-center justify-between h-16 gap-4">
                ...
                <NavigationMenuList className="flex items-center space-x-1">
                  {dashboardNavItems.map((item) => (
                    <NavigationMenuItem key={item.href} className="relative">
                      <NavigationMenuTrigger className="h-9 px-2 text-xs bg-transparent hover:bg-accent/50 data-[state=open]:bg-accent/60 transition-colors whitespace-nowrap">
                        <item.icon className="h-4 w-4 mr-1" />
                        <span className="hidden 2xl:inline">{item.title}</span>
                        <span className="2xl:hidden">{item.title.substring(0, 4)}</span>
                      </NavigationMenuTrigger>
                      <NavigationMenuContent className="absolute top-full left-0 mt-1 p-2 min-w-[200px] bg-background border border-border rounded-md shadow-lg z-50">
        ...
        <div className={`xl:hidden absolute top-full left-0 right-0 border-t border-border/60 bg-background/95 backdrop-blur-xl transition-all duration-300 ease-in-out z-40 ${
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="py-2 space-y-1 max-h-[calc(100vh-4rem)] overflow-y-auto">{isDashboard ? (
                <>
                  {/* Mobile Search */}
